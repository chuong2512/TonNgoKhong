...
public class JoystickManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(GameStarting == true)
        {
            ...
            {
                float ps = playerSpeed+ (playerSpeed* supplies.playerSpeed)/100 + PlayerPrefs.GetFloat("Spead") / 10;
                Debug.Log("player speed" + ps);
                
                rb.velocity = new Vector2(joystickMovement.joystickVec.x * (shoesSpeed+playerSpeed + (playerSpeed * supplies.playerSpeed) / 100), joystickMovement.joystickVec.y * (shoesSpeed + playerSpeed + (playerSpeed * supplies.playerSpeed) / 100));
